...
; Medusa stores MD5 hash for each file in the metadata file, which it retrieves from the Azure Blob properties if
; using Azure Blob Storage. But Azure Storage Accounts don't calculate MD5 for files larger than 64MB so that 
; Medusa could not get MD5 from Azure Blob properties for such files. This option allows Medusa to calculate the MD5 hash 
; locally for those large files. By doing so, we can guarantee that each file will have an associated MD5 hash 
; in the metadata.
;azure_calculate_md5_for_large_files = False

; Before Cassandra 4.1, SSTable identifiers are not globally unique.
; https://cassandra.apache.org/_/blog/Apache-Cassandra-4.1-New-SSTable-Identifiers.html
; This option allows Medusa to append 32 random characters to each SSTable identifier to make it globally unique 
; in storage
